...
class GatewayStub implements MerchantReturnGateway {
  ...
  public list(actorId: string, limit: number) {
    void actorId;
    void limit;
  ...
  public receive(actorId: string, returnId: string, input: MerchantReturnCommandInput) {
    void actorId;
    void input;
    return Promise.resolve({ returnId, status: 'RECEIVED' });
  }
  public inspect(actorId: string, returnId: string, input: MerchantReturnInspectionInput) {
    void actorId;
    this.inspection = input;
...
describe('MerchantReturnService', () => {
  it('submits a complete typed inspection', async () => {
    ...
    const result = await service.inspect(CONTEXT, RETURN_ID, KEY, {
      items: [
        { returnItemId: ITEM_ID, inspectionStatus: 'SELLABLE', merchantDecision: 'ACCEPTED' },
      ],
    });
