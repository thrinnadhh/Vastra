...
class GatewayStub implements CaptainFinanceGateway {
  public listCod(status: string | null, limit: number) { void status; void limit; return Promise.resolve([]); }
  public reconcileCod(actorId: string, collectionId: string, input: ReconcileCodInput) { void actorId; void input; return Promise.resolve({ collectionId, status: 'RECONCILED' }); }
  public getPayoutEligibility(captainId: string, periodStart: string, periodEnd: string) { void periodStart; void periodEnd; return Promise.resolve({ captainId, eligible: true }); }
  public createPayout(actorId: string, input: CreateCaptainPayoutInput) { void actorId; return Promise.resolve({ payoutId: ID, captainId: input.captainId }); }
  public getPayout(payoutId: string) { return Promise.resolve({ payoutId }); }
}
