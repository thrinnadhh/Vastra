  ...
  public async list(
    ...
    try {
      ...
      if (next !== null) {
        ...
        nextCursor = {
          createdAt: requireTimestamp(next, 'updatedAt'),
          id: requireString(next, 'id'),
        };
      }
      return {
        items: await this.resolutionService.resolveMany(
          context.actor.id,
          raw['items'].map(parseSavedLook),
        ),
  ...
  public async get(context: AuthenticatedRequestContext, lookIdValue: unknown): Promise<SavedLook> {
    try {
      ...
      return await this.resolutionService.resolve(
        context.actor.id,
        parseSavedLook(
          await this.gateway.execute('get_saved_look', {
            p_actor: context.actor.id,
            p_look_id: lookId,
          }),
        ),
