...
class IntegrationMerchantShopContextGateway implements MerchantShopContextGateway {
  public findOwnedShops(client: SupabaseClient): Promise<readonly MerchantCatalogueShopSnapshot[]> {
...
describe('merchant catalogue shop context integration', () => {
  ...
  beforeAll(async () => {
    ...
    const testingModule = await Test.createTestingModule({
      ...
      .useValue(new IntegrationMerchantShopContextGateway())
      .compile();
  ...
  it('lists the ready merchant catalogue shop scope', async () => {
    const response = await request(httpServer)
      .get('/merchant/catalogue/shops')
  ...
  it('returns an owned shop by identifier', async () => {
    const response = await request(httpServer)
      .get(`/merchant/catalogue/shops/${SHOP_ID}`)
      .set('Authorization', 'Bearer merchant-ready-token');
  ...
  it('hides a shop outside the merchant scope', async () => {
    const response = await request(httpServer)
      .get(`/merchant/catalogue/shops/${OTHER_SHOP_ID}`)
      .set('Authorization', 'Bearer merchant-ready-token');
  ...
  it('rejects malformed shop identifiers canonically', async () => {
    const response = await request(httpServer)
      .get('/merchant/catalogue/shops/not-a-uuid')
      .set('Authorization', 'Bearer merchant-ready-token');
  ...
  it('denies customers before catalogue data lookup', async () => {
    const response = await request(httpServer)
      .get('/merchant/catalogue/shops')
  ...
  it('keeps pending merchants out of operational catalogue routes', async () => {
    const response = await request(httpServer)
      .get('/merchant/catalogue/shops')
  ...
  it('requires authentication', async () => {
    const response = await request(httpServer).get('/merchant/catalogue/shops');
