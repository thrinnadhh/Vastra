...
import type { CustomerMobileEnvironment } from './mobile-environment';
import type {
  AuthSessionEvent,
  AuthSessionPort,
  RestorableSession,
...
export function createCustomerSupabaseClient(environment: CustomerMobileEnvironment) {
...
export type CustomerSupabaseClient = ReturnType<typeof createCustomerSupabaseClient>;
...
function updateAutoRefresh(client: CustomerSupabaseClient, state: AppStateStatus): void {
  if (state === 'active') {
...
export function startSupabaseAuthLifecycle(client: CustomerSupabaseClient): () => void {
  if (Platform.OS === 'web') {
    return () => undefined;
  }
