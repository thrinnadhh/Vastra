...
if (bundleResult.status !== 0) {
  if (bundleResult.stdout.length > 0) console.error(bundleResult.stdout);
  if (bundleResult.stderr.length > 0) console.error(bundleResult.stderr);
  ...
  const resolveReference = (node) => {
    if (!isRecord(node) || typeof node.$ref !== 'string' || !node.$ref.startsWith('#/')) return node;
  ...
  const schemaToType = (input) => {
    ...
    if (Array.isArray(schema.oneOf) && schema.oneOf.length > 0) return schema.oneOf.map(schemaToType).join(' | ');
    if (Array.isArray(schema.anyOf) && schema.anyOf.length > 0) return schema.anyOf.map(schemaToType).join(' | ');
    if (Array.isArray(schema.allOf) && schema.allOf.length > 0) return schema.allOf.map(schemaToType).join(' & ');
    ...
    const typeParts = rawTypes
      ...
      .map((type) => {
        switch (type) {
          ...
          case 'array': return `readonly (${schemaToType(schema.items)})[]`;
          case 'object': {
            const required = new Set(Array.isArray(schema.required) ? schema.required : []);
            const properties = isRecord(schema.properties) ? schema.properties : {};
            const members = Object.entries(properties).map(
              ([name, propertySchema]) => `${quote(name)}${required.has(name) ? '' : '?'}: ${schemaToType(propertySchema)};`,
            );
            ...
            return members.length === 0 ? 'Readonly<Record<string, unknown>>' : `Readonly<{ ${members.join(' ')} }>`;
          }
          default: return 'unknown';
        }
      });
    if (typeParts.length > 0) return [...new Set(typeParts)].join(' | ');
    if (isRecord(schema.properties)) return schemaToType({ ...schema, type: 'object' });
    return 'unknown';
  };
  ...
  const requestType = (pathItem, operation) => {
    ...
    for (const [name, type] of groups) {
      if (type !== null) {
        const location = name === 'headers' ? 'header' : name;
        const required = name === 'path' || parameters.some((parameter) => parameter.in === location && parameter.required === true);
    ...
    if (isRecord(requestBody)) {
      ...
      if (media !== null) members.push(`body${requestBody.required === true ? '' : '?'}: ${schemaToType(media.schema)};`);
    }
    return members.length === 0 ? 'Readonly<Record<never, never>>' : `Readonly<{ ${members.join(' ')} }>`;
  };
  ...
  const runtimeResponses = (operation) => {
    const output = {};
    if (!isRecord(operation.responses)) return output;
    for (const [status, unresolvedResponse] of Object.entries(operation.responses)) {
      const response = resolveReference(unresolvedResponse);
      const content = isRecord(response?.content) ? response.content : {};
      const media = isRecord(content['application/json']) ? content['application/json'] : null;
  ...
  for (const [path, rawPathItem] of Object.entries(paths)) {
    ...
    for (const [method, rawOperation] of Object.entries(pathItem)) {
      ...
      if (typeof operationId !== 'string' || operationId.length === 0) {
        throw new Error(`Every OpenAPI operation requires operationId: ${method.toUpperCase()} ${path}`);
      }
      if (seenOperationIds.has(operationId)) throw new Error(`Duplicate OpenAPI operationId: ${operationId}`);
      seenOperationIds.add(operationId);
      const security = rawOperation.security ?? document.security;
      const requiresAuth = Array.isArray(security) && security.length > 0;
      const successTypes = responseSchemas(rawOperation, (status) => /^2(?:\d\d|XX)$/u.test(status));
      const errorTypes = responseSchemas(rawOperation, (status) => !/^2(?:\d\d|XX)$/u.test(status));
      operationEntries.push([operationId, {
        method: method.toUpperCase(), path, requiresAuth, responses: runtimeResponses(rawOperation),
      }]);
      operationTypes.push(`${quote(operationId)}: { request: ${requestType(pathItem, rawOperation)}; response: ${successTypes.join(' | ') || 'unknown'}; error: ${errorTypes.join(' | ') || 'unknown'}; };`);
    }
  }

  const schemas = isRecord(document.components?.schemas) ? document.components.schemas : {};
  const schemaMembers = Object.entries(schemas).map(([name, schema]) => `${quote(name)}: ${schemaToType(schema)};`);
  const runtimeOperations = Object.fromEntries(operationEntries.sort(([left], [right]) => left.localeCompare(right)));
